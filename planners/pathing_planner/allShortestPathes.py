...
"""
This script will find all shortest pathes between "entry" and the enterprise
or otherwise specified nodes on the network genereated by RNDgenerator.py
"""


def all_shortest_paths(nodeType, nodeSearchProperty, entryNodeName,
...
# Find path
    ...
    result = util.all_shortest_pathes(session, entry, exit, pathName, depth)
...
def main(nodeType="Machine", nodeSearchProperty="name",
         ...
         pathName="canAttack*", depth=10, __CLI__=False):
    return all_shortest_paths(nodeType, nodeSearchProperty, entryNodeName,
